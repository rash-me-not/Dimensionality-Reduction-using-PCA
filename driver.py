...
def pca_analysis(corpus_dir):
    ...
    for counter, value in enumerate(variance_captured):
        ...
        while value >= decile:
            print(
                "{} components required for >={}% ".format(counter + 1, decile))
    ...
    dft_intensity_6a, time_6a, freq_6a = plotSpectrogram.spectrogram([filename],
                                                                     10, 20)
    ...
    plt.figure()
    plt.title("Plot for woman/ac/6a.wav file in PCA Space")
    plt.pcolormesh(time_6a, np.arange(0, dimensions + 1),
                   (np.transpose(np.abs(eigen_vec_6a))))
